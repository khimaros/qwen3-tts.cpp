...
bool AudioCodecEncoder::load_model(const std::string & model_path) {
    ...
    for (int64_t i = 0; i < n_tensors; ++i) {
        ...
        if (sname.find(".usage") != std::string::npos) continue;
        ...
        // VQ codebooks
        if (sscanf(name, "tok_enc.vq_semantic.%d.codebook", &idx) == 1) {
            if (idx == 0) model_.vq_sem_codebook = tensor;
            continue;
        }
        if (sscanf(name, "tok_enc.vq_acoustic.%d.codebook", &idx) == 1) {
            if (idx >= 0 && idx < 31) model_.vq_acou_codebook[idx] = tensor;
            continue;
        }

        // CNN conv layers (indices 0, 3, 6, 9, 12, 14)
        if (sscanf(name, "tok_enc.conv.%d.weight", &idx) == 1) {
        ...
        if (sscanf(name, "tok_enc.conv.%d.bias", &idx) == 1) {
        ...
        if (sscanf(name, "tok_enc.res.%d.blk.%d.weight", &idx, &sub) == 2) {
            for (int s = 0; s < 4; ++s) {
        ...
        if (sscanf(name, "tok_enc.res.%d.blk.%d.bias", &idx, &sub) == 2) {
            for (int s = 0; s < 4; ++s) {
                if (idx == RESUNIT_LAYERS[s]) {
...
struct ggml_tensor * AudioCodecEncoder::apply_tfm_layer(struct ggml_context * ctx,
    ...
    // no causal mask for encoder (bidirectional self-attention)
...
struct ggml_cgraph * AudioCodecEncoder::build_graph(int32_t n_samples) {
    ...
    for (int s = 0; s < 4; ++s) {
        ...
        cur = ggml_pad_ext(ctx0, cur, left_pad, 0, 0, 0, 0, 0, 0, 0);
        ...
        seq_len = (seq_len + stride - 1) / stride;  // ceil division for tracking
    }
    ...
    // causal pad 2 for kernel=4, stride=2
    cur = ggml_pad_ext(ctx0, cur, 2, 0, 0, 0, 0, 0, 0, 0);
...
void AudioCodecEncoder::vq_encode(const float * features, int32_t n_frames,
    ...
    for (int f = 0; f < n_frames; ++f) {
        ...
        // project residual to acoustic space
        std::vector<float> acou_proj(dim);
        apply_proj(acou_in_proj.data(), residual.data(), acou_proj.data(), dim, hidden);
